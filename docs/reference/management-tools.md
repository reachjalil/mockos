...
Status: Generated from the implemented 24-tool management registry; F1 mock MCP is source-qualified locally; F2 mock-LLM definitions are management-only
...
## Mock LLM server definitions (management only)
...
- This is a configuration plane, not an LLM service. The OpenAI/Anthropic
  provider data plane remains `unavailable`,
  and deployed acceptance remains `unqualified`.
- Use [the mock LLM guide](../mock-llm.md) for the exact definition, revision,
  credential, schema-migration, cleanup, evidence, and unsupported boundaries.

## Unavailable MCP surfaces

- Mock OpenAI and Anthropic provider APIs remain unavailable even though their
  MCP-only server-definition operations are source-implemented.
