...
Status: Generated from the implemented 24-tool management registry; F1 mock MCP and bounded F2 OpenAI/Anthropic data planes are source-qualified locally
...
- It implements `GET /v1/models`, `GET /v1/models/{model}`, and non-streaming
  `POST /v1/messages`. Use authenticated `GET /v1/models` as the capability probe;
...
- Streaming, beta APIs, OpenAI Responses, conversation state,
  observations/assertions, Cloud pinning,
...
- OpenAI Responses, provider streaming/betas, durable conversations, and
