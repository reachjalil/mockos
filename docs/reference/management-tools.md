...
Status: Generated from the implemented 27-tool management registry; F1 mock MCP and bounded streaming OpenAI/Anthropic data planes are source-qualified locally
...
The current management MCP server exposes **27 tools**.
...
| [`put_mock_mcp_server`](#put_mock_mcp_server) | destructive | idempotent | request redact | MCP only |
...
| [`list_mock_mcp_blueprints`](#list_mock_mcp_blueprints) | read | safe | none | MCP only |
| [`get_mock_mcp_blueprint`](#get_mock_mcp_blueprint) | read | safe | none | MCP only |
| [`install_mock_mcp_blueprint`](#install_mock_mcp_blueprint) | destructive | idempotent | none | MCP only |
...
<a id="list_mock_mcp_blueprints"></a>

### `list_mock_mcp_blueprints`

**List mock MCP blueprints.** Lists the versioned built-in mock MCP server blueprints. Entries are documentation-derived, deterministic synthetic fixtures and do not contact provider networks or claim upstream output-wire parity.
          ...
          "items": {
            ...
            "properties": {
              ...
              "id": {
                "maxLength": 128,
                "minLength": 5,
                "pattern": "^[a-z0-9][a-z0-9-]*(?:\\/[a-z0-9][a-z0-9-]*){2,4}$",
                "type": "string"
              },
              "provenance": {
                "additionalProperties": false,
...
<a id="get_mock_mcp_blueprint"></a>

### `get_mock_mcp_blueprint`

**Get mock MCP blueprint.** Returns one built-in blueprint with its source provenance, fidelity limits, and complete deterministic synthetic server definition.
