...
Status: Accepted M3/M5 ledger plus sampled M6 deployment; bounded MSAL Node and Okta Auth JS local X/Q added; live-provider parity remains unverified
...
local Wrangler HTTPS socket for one custom-authority confidential-client
authorization-code + S256 PKCE flow, forced refresh, lifecycle disable, observable
request sequence, redaction, normal cleanup, and a focused ready-Worker `SIGTERM`
cleanup probe. That row is D/I/S/X/Q yes and H/V/P no; it does not inherit M3/M6
deployment evidence.
...
| Pinned official identity client | `@azure/msal-node` 5.4.2 custom-authority confidential-client flow: D/I/S/X/Q yes; H/V/P no | `@okta/okta-auth-js` 8.0.1 public-client custom-authorization-server flow: D/I/S/X/Q yes; H/V/P no | Thin provider wrappers configure the shared [actual-network harness](../../scripts/e2e-local-official-client.mjs) and [signal cleanup probe](../../scripts/e2e-local-official-client-cleanup.mjs). Entra uses host-only `knownAuthorities`, `ProtocolMode.OIDC`, code + S256 PKCE, forced refresh, and lifecycle disable/`AADSTS50057`. Okta uses `getWithRedirect`, `parseFromUrl` plus the exact Node `_getLocation` seam, code + S256 PKCE, JWKS ID-token verification, refresh rotation, functional owner-bound public revocation, and lifecycle suspend/`invalid_grant`. MCP discovery and runtime agree on the conditional registration contract. Both use exact request assertions, bounded credential evidence, normal cleanup, and focused ready-Worker `SIGTERM` cleanup. These are two local client/version/flow claims, not broad SDK, browser, hosted, or live-provider parity. |
...
| Request log and assertions | Accepted for M3 inbound and the tested M5 outbound sequence; MSAL OAuth sequence/redaction source-qualified | Accepted for M3 inbound and the tested M5 outbound sequence; Auth JS OAuth/JWKS/revocation sequence and redaction source-qualified | M5 adds response-body predicates, repeated non-overlapping ordered counts, and bounded/redacted outbound records. The MSAL local actual-network test matches six discovery/login/code/refresh/disabled-refresh requests once. The Auth JS test matches eight discovery/login/code/JWKS/refresh/revoke/suspended-refresh requests once, requires token statuses `[200, 200, 400]`, parses named sensitive fields as `[REDACTED]`, and rejects raw, URI-encoded, or form-encoded forms of every exercised credential/token. This does not classify arbitrary encodings or unknown fields. Neither client record has hosted or live-provider evidence. |
