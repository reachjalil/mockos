...
Status: Accepted M3/M5 ledger plus sampled M6 deployment; bounded MSAL Node local X/Q added; live-provider parity remains unverified
...
| Pinned official identity client | `@azure/msal-node` 5.4.2 custom-authority confidential-client flow: D/I/S/X/Q yes; H/V/P no | No pinned official Okta client qualification | The [actual-network harness](../../scripts/e2e-entra-msal.mjs), [official-client flow](../../scripts/e2e-entra-msal-client.mjs), and [signal cleanup probe](../../scripts/e2e-entra-msal-cleanup.mjs) use host-only `knownAuthorities`, `ProtocolMode.OIDC`, local Wrangler HTTPS, official MCP management, code + S256 PKCE, forced refresh, lifecycle disable/`AADSTS50057`, exact request assertion, secret-safe durable evidence, normal cleanup, and focused ready-Worker `SIGTERM` cleanup. This is one local client/version/flow, not broad MSAL, hosted, or live-provider parity |
...
| Authorization code | Implemented for hosted login; bounded MSAL Node 5.4.2 local X/Q | Implemented for hosted login | Both Worker tests cover login, redirect, code redemption, and one-time code use at the core boundary. The MSAL candidate additionally uses `getAuthCodeUrl` and `acquireTokenByCode` through a real local HTTPS socket; H/V/P remain no |
| PKCE S256 | Required and implemented; bounded MSAL Node 5.4.2 local X/Q | Required and implemented | Positive Worker round trips and core/adapter rejection coverage; plain PKCE is intentionally unsupported. The MSAL client generates and redeems its own S256 challenge/verifier over actual-network local Wrangler |
...
| Refresh tokens | Accepted for M3 HTTP redemption/family rotation; deployed raw-protocol sample plus bounded MSAL forced-refresh local X/Q | Accepted for M3 HTTP redemption and family rotation in the local/hosted suite; introspection/revocation remains implemented | [Core tests](../../packages/core/src/okta.test.ts) cover hashed storage, rotation, bounded scopes, atomic replacement, concurrent redemption, replay-family revocation, and lifecycle revocation. Entra and Okta adapter tests plus focused Worker round trips cover the refresh grant; the deployed M3 sample exercises raw Entra rotation, narrowing, signed identity, and lifecycle revocation. Separately, pinned MSAL calls `acquireTokenSilent` with `forceRefresh: true` before and after disable and receives `200` then `invalid_grant`/`AADSTS50057`; that client evidence is local only |
...
| Request log and assertions | Accepted for M3 inbound and the tested M5 outbound sequence; MSAL OAuth sequence/redaction source-qualified | Accepted for M3 inbound and the tested M5 outbound sequence | M5 adds response-body predicates, repeated non-overlapping ordered counts, and bounded/redacted outbound records. The MSAL local actual-network test matches discovery, login GET/POST, code, refresh, and disabled-refresh once; focused Worker coverage proves durable form/JSON token and redirect-location secrets are redacted. No MSAL hosted or live-provider evidence exists |
