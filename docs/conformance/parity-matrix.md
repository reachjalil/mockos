...
Status: Accepted M3 emulator ledger plus locally qualified M5 outbound source; live-provider parity remains unverified
...
M5 entries marked “source candidate” refer to the locally qualified source. Their Worker
and worker-kit suites, full repository gate, and two-process provisioning e2e are
green locally, but an immutable revision, hosted-CI record, and deployed gates remain
open; they are not “Accepted for M3” and do not inherit the M3 deployment record.
...
| Request log and assertions | Accepted for M3 inbound traffic; M5 ordered outbound assertion is a source candidate | Accepted for M3 inbound traffic; M5 ordered outbound assertion is a source candidate | The accepted environment source captures routed protocol requests. M5 core tests add response-body predicates and repeated non-overlapping ordered sequence counts, while worker-kit adds bounded/redacted outbound records. Worker/full local tests and the four-step process-e2e assertion are green; deployed evidence is pending |
...
| Group overage | M6 source candidate: inline through 200, claim source at 201 | Not implemented | Focused core, Worker, and fixture-executor tests cover the exact boundary, the 201-ID token probe, the trusted same-environment Graph `getMemberObjects` URL in path and subdomain shapes, and the fallback's 1,000-ID response ceiling backed by a 1,001-ID probe; broad Graph parity and deployed M6 evidence are not claimed |
| Outbound provisioning dialect | M5 source candidate: deterministic lookup/create/update, filtered email PATCH, group push, deactivate/delete, and explicit 429 follow-ups | M5 source candidate: deterministic lookup/create, PUT-heavy updates, optional group push, terminal lifecycle work, and explicit 429 follow-ups | [Planner/interpreter tests](../../packages/core/src/provisioning/provisioning.test.ts) exercise provider-specific operation shapes. The [Workflow source](../../packages/worker-kit/src/provisioning-workflow.ts), SSRF controls, target app, tool 15, and CLI command pass Worker/full local gates and a two-process Entra-shaped target flow. Hosted/deployed qualification and live-provider comparison remain pending |
...
The M5 outbound rows are deliberately outside that accepted M3 paragraph. Their
[local source qualification](../evidence/m5-local-source-qualification.md) is green,
but no recorded immutable M5 candidate, hosted CI run, or staging/production deployment
