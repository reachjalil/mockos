...
Status: Accepted M3/M5 ledger plus sampled M6 deployment; bounded MSAL Node authorization-code/public-device and Okta Auth JS local X/Q added; live-provider parity remains unverified
...
The Okta Auth JS candidate is bound separately to the
...
shared owned local Wrangler HTTPS and signal-cleanup boundaries for one public-client
authorization-code + S256 PKCE flow, JWKS-backed ID-token verification, refresh
rotation, owner-bound public access-token revocation, lifecycle suspend, observable
request sequence, redaction, normal cleanup, and ready-Worker `SIGTERM` cleanup probe.
That row is D/I/S/X/Q yes and H/V/P no; it does not inherit M3/M6 deployment evidence.
...
| Pinned official identity client | `@azure/msal-node` 5.4.2 custom-authority confidential-code and public-device flow: D/I/S/X/Q yes; H/V/P no | `@okta/okta-auth-js` 8.0.1 public-client custom-authorization-server flow: D/I/S/X/Q yes; H/V/P no | Thin provider wrappers configure the shared [actual-network harness](../../scripts/e2e-local-official-client.mjs) and [signal cleanup probe](../../scripts/e2e-local-official-client-cleanup.mjs). Entra uses host-only `knownAuthorities`, `ProtocolMode.OIDC`, code + S256 PKCE, a separate secret-free public device registration, one callback/immediate pending poll, credential-gated activation, forced refresh for both accounts, and lifecycle disable/`invalid_grant`; the confidential error also asserts `AADSTS50057`. Okta uses `getWithRedirect`, `parseFromUrl` plus the exact Node `_getLocation` seam, code + S256 PKCE, JWKS ID-token verification, refresh rotation, functional owner-bound public revocation, and lifecycle suspend/`invalid_grant`. MCP discovery and runtime agree on the conditional registration contract. Both use exact request assertions, bounded credential evidence, normal cleanup, and focused ready-Worker `SIGTERM` cleanup. These are two pinned local client records, not broad SDK, browser, hosted, or live-provider parity. |
...
| Authorization code | Implemented for hosted login; bounded MSAL Node 5.4.2 local X/Q | Implemented for hosted login; bounded Okta Auth JS 8.0.1 public-client local X/Q | Worker tests cover login, redirect, code redemption, and one-time code use at the core boundary. MSAL uses `getAuthCodeUrl` and `acquireTokenByCode`; Auth JS uses `getWithRedirect` and `parseFromUrl` through real local HTTPS sockets. Both client records remain H/V/P no. |
...
| ID-token provider claims | Implemented for the tested authorization-code and device scope sets | Implemented for minimal and profile/email/group scope shapes | The Entra corpus has 25 documented targets and 13 implemented cases. Device core tests require distinct Entra `uti` claims on access and ID tokens; M6 cases cover their named token edges. All Okta fixtures remain documented expectations. Automated tests cover only linked subsets. |
...
| Request log and assertions | Accepted for M3 inbound and the tested M5 outbound sequence; combined MSAL code/device sequence and redaction source-qualified | Accepted for M3 inbound and the tested M5 outbound sequence; Auth JS OAuth/JWKS/revocation sequence and redaction source-qualified | M5 adds response-body predicates, repeated non-overlapping ordered counts, and bounded/redacted outbound records. The MSAL local actual-network test matches its combined discovery/login/code/device/activation/refresh/lifecycle sequence once, requires token statuses `[200, 200, 200, 200, 400, 400, 400]`, parses exercised fields as `[REDACTED]`, and rejects raw, URI-encoded, or form-encoded password/secret/code/message/verifier/token representations. The Auth JS test matches eight discovery/login/code/JWKS/refresh/revoke/suspended-refresh requests once, requires token statuses `[200, 200, 400]`, and applies the same bounded representation checks to its exercised values. This does not classify arbitrary encodings or unknown fields. Neither client record has hosted or live-provider evidence. |
