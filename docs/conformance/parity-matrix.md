...
and Classic Authn source cases. It does not promote those cases to deployed or
...
| Deterministic broken tokens | M6 source candidate for five explicit mutations | Shared minting seam; Okta HTTP-grant equivalence is not claimed | The authenticated Worker/fixture executor covers `expired`, exact wrong audience, `not_yet_valid`, `bad_signature`, and exact wrong issuer. Each mutation is independently selected and the emitted JWT is inspected; source minting does not prove that a live provider grant emits the same token. Deployed and verified-live M6 evidence remain pending |
| Token-claim clock skew | M6 source candidate for bounded claim-only skew | Shared signing seam; no Okta fixture claim | An exact-only `token.before_sign` action shifts temporal claims by an integer within plus/minus 86,400 seconds without moving the environment clock or stored authorization/device/refresh timestamps. The plus-300-second fixture is consumed exactly once. This is source evidence only |
...
| Deterministic SCIM edges | M6 source candidate: conflict, race, and missing-schema tolerance | M6 source candidate: conflict, race, and singleton-Operations tolerance | The separate [M6 edge fixtures](../../packages/testkit/fixtures/m6/scim) and [HTTP suite](../../packages/engine-http/src/scim-edge-cases.test.ts) cover an injection-locked `409` without partial mutation, a soft-delete race with only tombstone-side effects, strict rejection before tolerance, and two exact case-specific repairs. A selected tolerance does not repair the other case, combined defects, unknown fields, invalid paths, missing values, or coercions. This row has source evidence only |
