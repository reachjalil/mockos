...
| F1 environment-hosted mock MCP | Bounded local D/I/S/X/Q; H/P and hosted/merge/deployment unqualified; V not applicable | Strict [mock-MCP contracts](../packages/contracts/src/mock-mcp.ts), bounded JSON Schema/templates/results, [declarative evaluation](../packages/core/src/behavior/evaluator.ts), schema-v6 revision/state/hash-only-session tables, the atomic [repository](../packages/core/src/mock-mcp/repository.ts), five MCP-only management operations, and the hand-written [`@mockos/mcp-mock`](../packages/mcp-mock) `2025-11-25` adapter pass their focused suites and the complete repository gate. Put requires `expectedRevision: null` for create or the positive current revision for changed replacement; canonical replay precedes CAS, and a changed replacement is a complete write that must resupply or rotate a Bearer Mock Credential. Reset/delete require the positive current revision and execute atomically: reset preserves definition/revision/sessions and retries as `cleared: 0`; delete removes definition/state/sessions, succeeds as `deleted: true`, and retries as typed `404`. Stale and delete/recreate ABA expectations return typed `409` without mutation. The [path-mode Worker integration](../apps/worker/test/mock-mcp.integration.test.ts) uses `@modelcontextprotocol/sdk` `1.29.0` to verify the 24-tool discovery schemas and exercises management create/replay/reset, one-winner concurrent replacement, stale put/reset/delete, delete/not-found, the routed data plane, separate credential/session boundary, pagination and all capability kinds, and redacted MCP observations; host-resolver tests separately cover both endpoint forms. That mounted official-client path qualifies bounded Q but is not actual-network evidence. State is capped per value and at 256 rows/2 MiB in aggregate. Result validation and HTTP-abort handling during configured latency happen before staged sequence commit; scripts fail closed. Resource-template reverse matching is bounded/linear and deterministic for empty, percent-encoded, and adjacent-variable captures. Message-level `notifications/cancelled` remains deliberately unenforced. The [F1 guide](./mock-mcp.md) records exact limits and gaps. Hosted CI, merge, package publication, deployment, wildcard TLS, private hosted integration, GET/list-change streaming, a second protocol version, scripts, proxy, and broader client/ecosystem conformance remain unqualified or unavailable. |
...
| MCP runtime | Accepted 15-tool M5 hosted slice; five-tool F1 bounded local D/I/S/X/Q; 24-tool current source registry | The handler-agnostic [registry and tests](../packages/mcp/src/index.test.ts) preserve `run_provisioning_cycle` as tool 15, append five F1 operations, and append four F2 definition operations. F1 discovery requires null-create-or-positive-replace intent on put and positive current revisions on reset/delete while the management registry remains exactly 24 tools and self-hosted HTTP remains five routes. The mounted Worker integration uses `@modelcontextprotocol/sdk` `1.29.0` to discover those schemas and invoke canonical replay, reset, concurrent replacement, typed stale conflicts, delete, and not-found replay, qualifying only that bounded local path. The SDK 1.29 `tools/list` regression for `create_application` separately proves defaulted inputs remain optional, an explicit public `if`/`then` forbids a secret and narrows grants, and exact output branches expose a secret only for confidential registrations. Authenticated mounted Worker tests, the built-CLI process e2e, and staging/production hosted starts passed for the historical M5 set only. The two actual-network identity-client harnesses use MCP for setup, discovery, observation, lifecycle, and cleanup. No F1/F2 addition inherits deployed evidence, the F1 mounted-client run is not actual-network evidence, the client runs do not re-exercise every tool remotely, and npm distribution remains open. |
...
The bounded F1 transport/runtime and definition-management CAS are locally
D/I/S/X/Q in the revision carrying this document: focused suites, the complete
repository gate, and the mounted official MCP SDK `1.29.0` Worker integration are
green. Q is limited to that named mounted flow; F1 has no actual-network, hosted,
deployed, or P evidence; V is not applicable to the synthetic flow. F2 adds
