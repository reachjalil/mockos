...
source and metadata-observation slice, and bounded Entra MSAL Node
...
CI, deployed-version, or console acceptance evidence yet. The Entra MSAL Node tranche
and dedicated Okta Auth JS Classic factor-to-OIDC tranche are separate local
actual-network and official-client qualifications; they do not inherit earlier
hosted or deployed evidence. The F0 contracts,
management operation registry, checked OpenAPI/client artifacts, and disabled wrapper
boundaries also pass the full local repository gate in the revision carrying this
document. F0 has no hosted CI, deployment, or distribution evidence yet and does not
activate an experimental runtime. The
[locally source-qualified F1 implementation](./f-series/f1-mcp-foundation.md) adds the
hand-written `2025-11-25` adapter, five server-definition/state tools, three built-in
blueprint list/get/install tools, revisioned state/hash-only sessions, declarative
evaluation, path/subdomain routing, and MCP-aware request assertions. Its first
secret-free preset, `salesforce/hosted-mcp/sobject-reads`, was derived from Salesforce
Hosted MCP SObject Reads documentation reviewed on 2026-07-25 and installs six ordered
deterministic Salesforce-style data tools. Its source availability guards include
request-local repeated-work caches, incremental 256-KiB UTF-8 template rendering, and
a sticky JSON Schema work budget whose tool-input exhaustion is a bounded `isError`
result rather than behavior execution. Its focused layer suites and complete
repository `pnpm check` gate are green in the revision carrying this document. Hosted
CI, merge, package publication, deployment, private Cloud consumption, Salesforce
network/live comparison, and broader ecosystem compatibility remain separate claims
until recorded. The
...
MCP-only operations, baseline environment persistence, mandatory revision
...
hosted-smoke (H), verified-live (V), and production-ready (P). The Entra and dedicated
Okta factor-to-OIDC official-client tranches are D/I/S/X/Q yes and H/V/P no. Existing
exact-version workers.dev records may
establish H for their raw-protocol samples without establishing Q, V, or P. No current
fixture or milestone is V or P.
...
| Core migrations and directory repositories | Accepted through M5; bounded M6 source suite and deployed sample green; additive Okta factor bridge locally source-qualified | The accepted [core substrate](../packages/core/src/core.test.ts), [directory/lifecycle](../packages/core/src/directory-lifecycle.test.ts), [Okta OAuth](../packages/core/src/okta.test.ts), and [scenario/log](../packages/core/src/scenario-log.test.ts) cover the M3 engine. The M5 [provisioning planner/interpreter](../packages/core/src/provisioning) passed focused/full and controlled-target gates. M6 focused tests cover the bounded [Classic Authn service](../packages/core/src/authn/okta-authn.ts), deterministic broken-token mutations, active/successor key handling, private-key scrubbing, bounded retention, and stale-instance/sign-rotate races. Authn state retrieval uses a sliding five-minute expiry while session capabilities retain their fixed five-minute issuance expiry. Each table is capped at 10,000 retained rows and each User at 32 retained rows per kind; ordered oldest-expiry eviction and a 256-row-per-table issuance GC pass are index-backed within the baseline schema. The additive slice verifies the returned synthetic factor with fixed passcode `000000`, preserves state after `E0000068`, consumes successful valid-password state atomically, and lets a validated OIDC authorization consume the resulting session capability once. A simultaneously password-expired User advances to `PASSWORD_EXPIRED` with the same live state/no session; factor replay returns `E0000011` without deleting that cancellable transaction. The slice does not change token `acr`/`amr`. The [M6 deployed sample](./evidence/m6-workers-dev-smoke.md) exercises the original externally observable slices, not this factor/session bridge or every source-only retention/concurrency/security assertion. |
...
| Fixture schema, loader, and runner | Complete for M3 SCIM; bounded device/M6 executors and sampled M6 deployment green; two Okta factor fixtures locally source-tested | The [schema](../packages/testkit/fixtures/fixture.schema.json), [loader](../packages/testkit/src/fixtures.ts), and [runner](../packages/testkit/src/runner.ts) load 38 Entra and 22 Okta source-reviewed OIDC fixtures, 113 SCIM fixtures, five historical M6 Okta Authn fixtures, and two additive factor-verification fixtures. The [Entra device executor](../packages/engine-http/src/entra-device-fixtures.test.ts) runs fixtures 23–27 against a core-backed HTTP composition; mounted [Worker coverage](../apps/worker/test/entra-device.integration.test.ts) exercises creation, pending/slow-down, credential-gated denial, approval/token/refresh/consumption, and unknown code, but not deterministic expiry. The [M6 Worker executor](../apps/worker/test/token-fixtures.integration.test.ts) supplies authenticated MCP/session, environment, client, subject, and Graph-bearer setup for Entra fixtures 31–38; the [Authn executor](../packages/engine-http/src/okta-authn-fixtures.test.ts) runs all seven Authn cases. The historical M6 smoke sampled all six M6 areas but was not a remote execution of this expanded source index. The other OIDC fixtures remain documented targets, and no live-provider comparison is claimed. |
| Provider fixture corpora | Mixed evidence | The 38 [Entra OIDC](../packages/testkit/fixtures/entra/oidc) fixtures contain 25 documented targets and 13 implemented cases: five device cases run through a core-backed HTTP executor, while eight M6 token/key/overage cases run through the local Worker. The mounted device Worker suite covers the non-expiry flow; fixture 27 expiry remains deterministic HTTP-only. All 22 [Okta OIDC](../packages/testkit/fixtures/okta/oidc) fixtures remain documented. The SCIM corpus contains 113 source-implemented cases. Seven [Okta Authn fixtures](../packages/testkit/fixtures/okta/authn) are source-reviewed and marked `implemented`: the five historical M6 cases plus factor success and invalid passcode. The two additions receive no historical M6 H evidence. None claims comparison with a live provider tenant. |
| Entra OIDC runtime | Accepted M3 slice; bounded device local D/I/S/X/Q; bounded M6 token/key/Graph slice sampled in deployment | [Core](../packages/core/src/core.test.ts), [Worker OIDC](../apps/worker/test/oidc.integration.test.ts), and [lifecycle cascade](../apps/worker/test/lifecycle-cascade.integration.test.ts) tests cover the accepted M3 hosted-login, authorization-code, refresh/lifecycle, Entra-claim, and RS256/JWKS slice; the [M3 deployed smoke](./evidence/m3-workers-dev-smoke.md) exercised those principal paths. The device tranche adds public-only 900-second/five-second authorization, credential-gated approve and deny at `/devicelogin`, Microsoft error names, atomic one-winner consumption with token persistence, trusted Graph-overage routing, unique access/ID-token `uti`, public refresh rotation, lifecycle rejection, and strict device evidence redaction. Fixtures 23–27 execute against a core-backed HTTP composition; mounted Worker and actual-network MSAL 5.4.2 cover the non-expiry path. M6 source tests add deterministic broken tokens, skew, one-key rollover overlap, and group overage with trusted same-environment Graph fallback capped at 1,000 returned IDs. The [M6 deployed sample](./evidence/m6-workers-dev-smoke.md) exercises rotation/stale-JWKS verification, plus-300-second skew, all five broken variants, and the exact path-mode 200/201 boundary. Current OIDC discovery and `get_wellknown_urls` omit UserInfo because no route exists; Entra fixtures 28 and 29 remain documented targets only. The device path has no hosted evidence, and deterministic expiry is not mounted. The 1,001-ID ceiling, concurrency, private-key scrubbing, subdomain routing, broader error fidelity, and verified-live comparison remain source-only or open. |
| Entra MSAL Node client | Bounded combined local D/I/S/X/Q passed; H/V/P unqualified | Pinned `@azure/msal-node` 5.4.2, `@modelcontextprotocol/sdk` 1.29.0, and Wrangler 4.114.0 traverse an actual local Wrangler HTTPS socket. The official MCP client creates one confidential code application with its real callback URI and one redirect-free public device application with `redirectUris: []`, then observes and cleans up the environment. MSAL uses the request-derived custom authority with host-only `knownAuthorities` and `ProtocolMode.OIDC`; it completes code + S256 PKCE, a single device callback, an immediate pending poll, credential-gated activation, device redemption, and forced refresh for both accounts. After lifecycle disable, both refreshes return `invalid_grant`; the confidential path also asserts `AADSTS50057`. A complete-array assertion requires exactly 14 provider method/path/status triples, including separate confidential and public discovery, with token statuses `[200, 200, 200, 200, 400, 400, 400]`. One ownership-checked local `/health` probe bridges Wrangler's loopback during the required five-second wait; it is outside provider evidence and is not a token retry. Named fields plus raw/URI/form representations of the password, secret, codes, repeated device message, verifier, and tokens are absent from durable evidence. See the [quickstart](./quickstarts/entra-msal-node.md) and [local record](./evidence/entra-msal-node-local-qualification.md). No remote Worker, hosted Cloud, real Entra tenant, broad MSAL matrix, or production-readiness result is claimed. |
| Okta runtime | Accepted M3 implementation plus bounded M6 Classic Authn deployed sample; additive factor/session bridge local D/I/S/X/Q | Existing [core](../packages/core/src/okta.test.ts), [HTTP adapter](../packages/engine-http/src/okta.test.ts), and [Worker integration](../apps/worker/test/okta.integration.test.ts) cover the M3 OAuth/device/directory slice. M6 adds core/HTTP/Worker coverage for `SUCCESS`, `MFA_REQUIRED`, `PASSWORD_EXPIRED`, explicit `LOCKED_OUT`, state retrieval/cancellation, hash-only capabilities, replay denial, lifecycle and SCIM-password-change revocation, and issuance-race denial. The additive slice mounts the returned factor-verification link, accepts only fixed synthetic passcode `000000`, preserves state after wrong factor/passcode `E0000068`, and issues a one-use session capability after atomic valid-password success. A simultaneous password-expired policy returns `PASSWORD_EXPIRED` with the same live state/no session; factor replay returns `E0000011` without deleting the cancellable transaction. The Okta authorize route validates client/redirect/scope/response type/S256 before consuming a session and returns a direct code callback; replay is `invalid_grant`. This is deterministic factor chaining, not RFC 6238/TOTP or a Sessions API/cookie implementation. Tokens retain `acr: urn:okta:loa:1fa:any` and `amr: [pwd]`. Unsupported password-change and unlock links are omitted. Successful retrieval slides state expiry; session expiry remains fixed. The HTTP shape uses singular `_embedded.factor` (an array), omits `passwordChanged`, restricts CORS, and redacts Authn/factor capabilities. Current OIDC discovery and `get_wellknown_urls` omit UserInfo. The [M6 smoke](./evidence/m6-workers-dev-smoke.md) samples only the original initial states/retrieval/CORS/privacy/redaction; it does not qualify the additive bridge. Verified-live comparison and the rest of the Classic machine remain open. |
| Okta Auth JS Classic factor-to-OIDC public client | Bounded local D/I/S/X/Q passed; H/V/P no | The dedicated qualification pins `@okta/okta-auth-js` 8.0.1 and `@modelcontextprotocol/sdk` 1.29.0. MCP creates an MFA-required User and secret-free public code/refresh application. Auth JS uses `signInWithCredentials`, follows the returned factor's SDK verification function with fixed synthetic `000000`, passes the resulting one-use `sessionToken` to `getWithRedirect`, and continues through S256 PKCE, `parseFromUrl` with the exact Node `_getLocation` seam, JWKS-backed ID-token verification, refresh rotation, public access-token revocation, and lifecycle `invalid_grant` after `suspend`. The accepted run matched exactly nine provider requests in Authn/factor/discovery/authorize/code/JWKS/refresh/revoke/failed-refresh order and token statuses `[200, 200, 400]`, plus structural and raw/URI/form redaction for password, Authn state/passcode/session capability, code/verifier, and tokens. See the [quickstart](./quickstarts/okta-auth-js-node.md) and [local record](./evidence/okta-auth-js-local-qualification.md). No RFC 6238, MFA-assurance claims, browser, hosted, real-Okta, broad SDK, or P result is claimed. |
| Scenario injection and request log | Accepted through the tested M5 assertion slice; M6 actions sampled in deployment; bounded F1/F2 metadata source-qualified | Accepted M3 [scenario/log tests](../packages/core/src/scenario-log.test.ts) cover deterministic scenarios, filtering, retention, and assertions. M5 response-body predicates, repeated non-overlapping ordered counts, Worker capture/redaction, local process e2e, and both hosted four-request assertions passed. M6 key rotation and clock skew are restricted to exact internal `token.before_sign` evaluation rather than the generic catch-all; both were sampled by the exact-version M6 deployed smoke. F1 adds exact MCP metadata. The baseline schema carries metadata-only OpenAI/Anthropic pending/terminal lifecycle rows plus exact query/count/sequence matchers, tested in core and mounted Worker/MCP integration. F1/F2 additions inherit no M5/M6 hosted or deployed evidence, and F2 observation is fail-open rather than an audit guarantee. |
...
| MCP runtime | Accepted 15-tool M5 hosted slice; eight-tool F1 bounded local D/I/S/X/Q; 27-tool current source registry | The handler-agnostic [registry and tests](../packages/mcp/src/index.test.ts) preserve `run_provisioning_cycle` as tool 15, append all eight F1 operations in canonical order (five server-definition/state operations followed by three built-in-blueprint operations), then append four F2 definition operations. F1 put/install use null-create-or-positive-replace intent; identical canonical replay, including identical delete/recreate convergence, resolves before CAS while only changed stale/ABA intent conflicts. Reset/delete require positive current revisions. Put/install are destructive when replacement changes the definition because they delete application state and terminate revision-bound sessions. Bearer-definition validation accepts the raw credential only at `authentication.token`, rejects its exact value elsewhere, and fails closed on dependency reflection; install verifies the complete public server spec. The registry remains exactly 27 tools and self-hosted HTTP remains five routes. Mounted Worker integrations use `@modelcontextprotocol/sdk` `1.29.0` to exercise custom-server and Salesforce-blueprint discovery, CAS, runtime, observation, and cleanup, qualifying only those bounded local paths. The SDK 1.29 `tools/list` regression for `create_application` separately proves defaulted inputs remain optional, an explicit public `if`/`then` forbids a secret and narrows grants, and exact output branches expose a secret only for confidential registrations. Authenticated mounted Worker tests, the built-CLI process e2e, and staging/production hosted starts passed for the historical M5 set only. The two actual-network identity-client harnesses use MCP for setup, discovery, observation, lifecycle, and cleanup. No F1/F2 addition inherits hosted/deployed/Cloud-pin/P evidence, the F1 mounted-client runs are not actual-network evidence, Salesforce source review is not V evidence, the client runs do not re-exercise every tool remotely, and npm distribution remains open. |
...
| Live-provider parity harness | Foundation source-complete; no live capture | The comparison machine is built and unit-tested: a client-side observation interceptor, a normaliser that replaces volatile values rather than deleting them and decodes JWTs so claim sets compare, a comparator that classifies every difference against a checked-in divergence ledger, a 28-case scenario matrix, and a drift-checked report generator. A mock-versus-mock self-test exercises the whole pipeline with no tenant, credential, or network. The ledger seeds 25 divergences from source review, 18 intentional and 7 tracked defects, including two conflicting Entra error catalogs that cannot both match real Entra. No real tenant has been contacted: all 28 cases are `not-captured`, no fixture is `verified-live`, and the suite is advisory so it never blocks a release. D/I/S only; X/Q/H/V/P unqualified. See the [roadmap](./conformance/live-parity-roadmap.md) |
...
versus fixed session expiry, caps/ordered eviction/256-row GC, baseline-compatible
...
The client completes authorization code with S256 PKCE and a separate public
device-code flow, validates account/tenant/token claims, and forces successful refresh
for both accounts. The device callback runs once, returns 900/5 policy without a
complete URI, and is activated only after the immediate pending poll. Lifecycle
disable makes both forced refreshes fail with `invalid_grant`; the confidential path
also asserts `AADSTS50057`. It matches the exact combined provider sequence, proves
structural plus raw/URI/form credential and token redaction, deletes the environment,
closes MCP, and removes the temporary CA. Focused Worker tests independently protect
the persistent redaction boundary. A separate prior `SIGTERM` cleanup probe observes
parent exit `143`, releases the provider and inspector ports, removes the detached
Wrangler process group, and removes temporary state. Distinct explicit inspector ports
allow the Entra and Okta qualifications and cleanup probes to run concurrently.

`pnpm e2e:entra-msal` passed for the combined flow under Node 26.4.0. The shared
ready-Worker `pnpm e2e:entra-msal-cleanup` record remains green from Node 24.18.0;
focused source tests also pass. Exact output is in the
...
## Okta Auth JS Classic factor-to-OIDC local client qualification

The qualified local slice pins `@okta/okta-auth-js` 8.0.1 and uses an explicit public
application registration. Public clients store and return no secret, reject a
...
environment. Auth JS calls `signInWithCredentials`, follows the returned factor's
verification function using fixed synthetic passcode `000000`, and supplies the
resulting one-use `sessionToken` to `getWithRedirect`. The direct authorization
callback continues through S256 PKCE and `parseFromUrl`, which exchanges the code and
verifies the RS256 ID token from discovery/JWKS. The Node harness supplies the
SDK-exposed `_getLocation` seam because the method otherwise dereferences `window`;
this is part of the exact 8.0.1 Node qualification, not a browser or version-range claim.
`renewTokens` rotates the refresh token, and `token.revoke` returns 200 for the
refreshed access token through public client-ID-only Basic compatibility. The
following suspension revokes exactly one remaining access token, proving the SDK
call changed state; a later SDK refresh fails with `invalid_grant` and
`User account is disabled.`.

The dedicated commands are `pnpm e2e:okta-mfa-authjs` and
`pnpm e2e:okta-mfa-authjs-cleanup`. The [qualification
record](./evidence/okta-auth-js-local-qualification.md) contains their exact final
output. The accepted run matched nine provider requests,
token statuses `[200, 200, 400]`, functional revocation, structural and raw/URI/form
redaction for the Authn/OIDC capabilities, environment deletion, process-group exit,
released provider/inspector ports, and temporary-state removal. The static passcode is
not RFC 6238/TOTP verification, and the tokens retain one-factor `acr` and
password-only `amr`; no MFA-assurance claim follows. No hosted-CI execution, remote
Worker, browser, real Okta organization, verified-live comparison, or P result is
claimed.
...
Durable Object RPCs are additive within the baseline schema. The full local `pnpm check`
...
The bounded F1 transport/runtime, definition-management CAS, and built-in Salesforce
blueprint loop are locally D/I/S/X/Q in the revision carrying this document: focused
suites, the complete repository gate, and the mounted official MCP SDK `1.29.0`
Worker integrations are green. Q is limited to those named mounted flows; F1 has no
actual-network, hosted, deployed, Cloud-pin, or P evidence. V is not applicable to
the generic synthetic engine, while the provider-derived Salesforce preset's V
dimension is explicitly unqualified: its 2026-07-25 documentation review is
provenance/design input, not live-provider comparison. F2 adds
