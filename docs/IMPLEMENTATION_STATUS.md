...
Status: M0-M3 accepted; M5 and the bounded M6 Classic Authn slice are locally qualified source pending milestone acceptance
...
`8645f405d5e3b922c30d51339b8b27f9fe30d93e`; the locally qualified M5/M6 source
...
| Contracts v0 | Accepted for M3; M5/M6 source candidates locally green | The accepted contract includes SCIM resources, directory lifecycle state/actions/results, 14 M3 MCP tools, and provider directory URLs. The M5 [provisioning contract](../packages/contracts/src/provisioning.ts) and tests add safe target metadata, snapshots, watermarks, plans, operations, responses, and `run_provisioning_cycle` as tool 15. The bounded M6 Authn slice adds seeded `passwordState` and request-derived `oktaAuthnEndpoint`. These remain unreleased APIs pending milestone qualification. |
...
| Okta runtime | Accepted M3 implementation plus bounded M6 Classic Authn source candidate; deployed directory subset | Existing [core](../packages/core/src/okta.test.ts), [HTTP adapter](../packages/engine-http/src/okta.test.ts), and [Worker integration](../apps/worker/test/okta.integration.test.ts) cover the M3 OAuth/device/directory slice. M6 adds core/HTTP/Worker coverage for `SUCCESS`, `MFA_REQUIRED`, `PASSWORD_EXPIRED`, explicit `LOCKED_OUT`, state retrieval/cancellation, five-minute hashed capabilities, replay denial, lifecycle invalidation, privacy, and Authn log redaction. Factor verification and the rest of the Classic machine are absent; the deployed M3 smoke is not M6 evidence. |
...
| Cloudflare Worker / Durable Object | Accepted and deployed for M3; M5/M6 Worker source locally qualified | Exact candidate `8645f405d5e3b922c30d51339b8b27f9fe30d93e` passed the M3 smoke. M5 adds provisioning composition. The bounded M6 slice mounts Authn separately from SSWS-protected management routes and redacts Authn credentials/capabilities from logs. The expanded Worker suite (24/24) and worker-kit suite (79/79) are green locally; no M6 hosted or deployed evidence exists. |
