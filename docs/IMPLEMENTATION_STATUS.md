...
M6 is not accepted. This integration source contains bounded Okta Classic Authn and
Entra token/key/Graph streams: five executable Authn fixtures; eight locally
Worker-executed token/key/overage fixtures; deterministic broken-token, skew, and
rotation behavior; a bounded same-environment Graph fallback; and their focused
core/HTTP/Worker evidence. These remain source-candidate results. They do not inherit
M3 or M5 hosted/deployed evidence and cannot be presented as completion of M6,
live-provider parity, or deployed acceptance.
