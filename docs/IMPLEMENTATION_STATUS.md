...
Status: M0-M3/M5 accepted; bounded M6 deployed; M7/F0 complete in source; F1 local; F2 management-only source slice
...
The [task guide](./mock-llm.md) records the exact management contract. There is still
no provider route/auth enforcement, model renderer/catalog, runtime/conversation
state or reset, edge pacing, observation, Wrangler/deployed conformance, or private
Cloud pin. F2 therefore remains incomplete and mock LLM APIs remain unavailable as an
operable data plane.
...
| F2 mock-LLM kernel and management definitions | Partial source implementation; no provider-data-plane qualification | The [F2 kernel record](./f-series/f2-llm-kernel.md) maps the bounded/deep-frozen plan, staged behavior adapter, pure OpenAI Chat Completions and Anthropic Messages JSON/SSE-frame renderers, and official-SDK in-process tests. The [management guide](./mock-llm.md) maps strict bounded definitions, four MCP-only operations, schema-v7 persistence with canonical writes, monotonic revisions, mandatory put/delete CAS with idempotent put replay, full strict-key resupply on replacement, provider-key hashing, secret-safe validation, bounded static-plan checks, platform-key-substring rejection, and non-round-trippable safe views. Focused migration tests prove v6-to-v7 upgrade and older-v6 refusal of a v7 store. The current registry is 24 tools and HTTP remains five routes. The injected Fetch harness alone supplies model-list/retrieve fixtures; there is no provider listener/route/parser/auth enforcement, model renderer/catalog, runtime/conversation state, reset, network pacing, request observation, Wrangler qualification, Cloud pin, or deployment. This does not satisfy the F2 exit gate. |
...
focused kernel tests plus MCP-only persisted server definitions; it is not a routed or
hosted provider runtime. Hosted CI, merge, package publication, deployment, private
Cloud integration, the remaining F2 data plane, F3 runtime, F4 authorization/audit,
broader client/ecosystem conformance, and F6 Code Mode activation remain pending.
