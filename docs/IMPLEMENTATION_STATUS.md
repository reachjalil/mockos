...
Status: M0-M3/M5 accepted; bounded M6 deployed; M7/F0 complete in source; F1 local; F2 partial
...
recorded.
The [partial F2 LLM kernel](./f-series/f2-llm-kernel.md) adds provider-neutral
response-plan schemas, a behavior-to-plan adapter, pure OpenAI/Anthropic
JSON/SSE-frame renderers, and in-process consumption tests with pinned official SDKs.
That focused source evidence does not add an LLM server definition, management MCP
tool, HTTP/provider route, authentication, persistence, edge pacing, observation,
Worker composition, private Cloud integration, or Wrangler/deployed conformance. F2
therefore remains incomplete and mock LLM APIs remain unavailable as an operable
product surface.
...
| F2 provider-neutral LLM response kernel | Partial local source evidence; no service/runtime qualification | The [F2 kernel record](./f-series/f2-llm-kernel.md) maps the bounded/deep-frozen mock-LLM plan, staged behavior adapter, pure OpenAI Chat Completions and Anthropic Messages JSON/SSE-frame renderers, and official-SDK in-process tests. Its focused suites and complete forced repository gate are green locally. The injected Fetch harness alone supplies model-list/retrieve fixtures; there is no model renderer or catalog. No listener, route, parser, auth check, Worker, Durable Object, persistence, network edge stream/pacing, request observation, management MCP operation, or Cloud consumer participates. This is not Wrangler or deployed conformance, does not increase the 20-tool management registry, and does not satisfy the F2 exit gate. |
...
| MCP runtime | Accepted 15-tool M5 hosted slice; 20-tool F1 registry source-qualified locally | The handler-agnostic [registry and tests](../packages/mcp/src/index.test.ts) preserve `run_provisioning_cycle` as tool 15 and append five locally source-qualified F1 management operations. Authenticated mounted Worker tests, the built-CLI process e2e, and staging/production hosted starts passed for the historical M5 set only. No F1 tool inherits that deployed evidence, and npm distribution remains open. |
...
this document: the focused suites and complete repository gate are green. The partial
F2 response kernel adds focused source tests only; it is not a routed or hosted
runtime. Hosted CI, merge, package publication, deployment, private Cloud integration,
the remaining F2 service, F3 runtime, F4 authorization/audit, broader
client/ecosystem conformance, and F6 Code Mode activation remain pending.
