...
| Fixture schema, loader, and runner | Complete for M3 local and hosted SCIM HTTP execution; eight M6 token fixtures execute locally | The [schema](../packages/testkit/fixtures/fixture.schema.json), [loader](../packages/testkit/src/fixtures.ts), and [runner](../packages/testkit/src/runner.ts) load 38 Entra and 22 Okta source-reviewed OIDC fixtures plus 113 SCIM fixtures. The [M6 Worker executor](../apps/worker/test/token-fixtures.integration.test.ts) supplies authenticated MCP/session, environment, client, subject, and Graph-bearer setup and executes Entra fixtures 31–38. This does not make the remaining OIDC fixtures executable. |
| Provider fixture corpora | Mixed evidence | The 38 [Entra OIDC](../packages/testkit/fixtures/entra/oidc) fixtures contain 30 documented targets and eight implemented, locally Worker-executed M6 token/key/overage cases; all 22 Okta OIDC fixtures remain documented. The SCIM corpus contains 91 RFC, 10 Entra, and 12 Okta cases; all 113 execute green locally. None claims comparison with a live provider tenant. |
...
| Scenario injection and request log | Accepted for M3; M5 assertions and M6 token actions are source candidates | Accepted M3 [scenario/log tests](../packages/core/src/scenario-log.test.ts) cover deterministic scenarios, filtering, retention, and assertions. The M6 token stream restricts key rotation and clock skew to exact internal `token.before_sign` evaluation, excluding the generic catch-all. Focused local tests are green; hosted/deployed M6 qualification remains pending. |
