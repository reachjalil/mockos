...
list/retrieve, OpenAI JSON/SSE Chat Completions, and non-streaming Anthropic Messages
through pinned official SDKs. Both auth modes require the dialect's valid provider
credential; strict mode checks only its current hash-only verifier. Anthropic
additionally requires exactly `anthropic-version: 2023-06-01` and rejects beta headers.
Configured midstream errors, OpenAI Responses, Anthropic streaming/betas,
runtime/conversation state or reset, LLM observation/assertion,
