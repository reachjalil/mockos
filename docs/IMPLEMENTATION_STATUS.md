...
source-complete, F1 is locally source-qualified, F2 is partial, and bounded Entra MSAL
Node local X/Q passed
...
[partial F2 LLM kernel](./f-series/f2-llm-kernel.md) adds provider-neutral response-plan
schemas, a behavior-to-plan adapter, pure OpenAI/Anthropic JSON/SSE-frame renderers,
and in-process consumption tests with pinned official SDKs. That focused source evidence
does not add an LLM server definition, management MCP tool, HTTP/provider route,
authentication, persistence, edge pacing, observation, Worker composition, private
Cloud integration, or Wrangler/deployed conformance. F2 therefore remains incomplete
and mock LLM APIs remain unavailable as an operable product surface.
