...
Status: M0-M3/M5 accepted; bounded M6 deployed; M7/F0 complete in source; F1 local; partial OpenAI F2 source-qualified locally
...
The bounded OpenAI request adapter, environment runtime, route composition, generated
provider manifest, and local Worker integration now source-qualify ordered model
list/retrieve and non-streaming Chat Completions through the pinned official OpenAI
SDK. Both auth modes require a provider Bearer value; strict mode checks the current
hash-only verifier. Anthropic routing, SSE, runtime/conversation state or reset, LLM
...
| F2 mock-LLM kernel, management definitions, and OpenAI data plane | Partial source-qualified implementation; hosted/deployed provider evidence unqualified | The [F2 kernel record](./f-series/f2-llm-kernel.md) maps the bounded/deep-frozen plan, behavior adapter, pure OpenAI/Anthropic projections, and exact provider boundary. The [task guide](./mock-llm.md) maps strict bounded definitions, four MCP-only operations, schema-v7 canonical persistence, monotonic revisions, mandatory put/delete CAS with idempotent put replay, full strict-key resupply, hash-only provider credentials, secret-safe validation, and non-round-trippable safe views. The executable OpenAI adapter adds exact path/subdomain routes, model list/retrieve, bounded non-streaming Chat Completions parsing, provider-shaped local/configured errors, fresh transport IDs, alternate/platform-key rejection, credential non-reflection, response limits, and abort-aware initial delay. The environment runtime hashes before reading the current definition, compares strict verifiers without early exit, derives stateless turns from prior assistant messages, and rechecks the revision before plan commit. Package suites plus the [local Worker integration](../apps/worker/test/mock-llm.integration.test.ts) configure through MCP and exercise pinned official-SDK model, text, tool, usage, error, isolation, and key-rotation flows. The generated [provider manifest](./reference/mock-llm-openai.v1.json) records the exact bounded contract. Management remains 24 MCP tools and five management HTTP routes. Anthropic routing, SSE, conversation/evaluator state, reset, LLM observations/assertions, Wrangler-network, hosted CI, Cloud pinning, deployment, and live-provider parity remain unavailable or unqualified, so the F2 exit gate is not satisfied. |
...
focused kernel/definition tests and a bounded routed OpenAI provider runtime that is
...
deployment, private Cloud integration, Anthropic, streaming/state/observation work,
