...
description: >-
  ...
  Okta environments, seed identities, register public or confidential OIDC clients, run
  ...
  up. Use when wiring or testing an application's enterprise identity integration or
  reproducing provider-shaped failures; do not
...
3. Call `create_application` with the exact callback URI, grants, and client type
   required by the test. `confidential` is the default: retain its returned synthetic
   `clientSecret` in memory or a test secret store and expect it only on creation.
