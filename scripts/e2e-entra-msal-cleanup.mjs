  ...
  new Promise((resolve, reject) => {
    ...
    server.listen({ host: "127.0.0.1", port: 0, exclusive: true }, () => {
      ...
      if (!address || typeof address === "string") {
        server.close();
        reject(new Error("Could not reserve a loopback cleanup-probe port."));
        return;
      }
      server.close((error) => {
...
const main = async () => {
  ...
  const temporaryParent = await mkdtemp(
    join(tmpdir(), "mockos-entra-msal-cleanup-probe-")
  );
  let child;
  ...
  let stderr = "";
  try {
    child = spawn(process.execPath, ["scripts/e2e-entra-msal.mjs"], {
      cwd: repositoryRoot,
      env: {
        ...process.env,
        MOCKOS_ENTRA_MSAL_E2E_CLEANUP_PROBE: "true",
        MOCKOS_ENTRA_MSAL_E2E_PORT: String(port),
        MOCKOS_ENTRA_MSAL_E2E_TEMP_PARENT: temporaryParent,
      },
    ...
    process.stdout.write(
      `${JSON.stringify({
        ok: true,
        claim: "entra-msal-e2e-signal-cleanup",
        signal: "SIGTERM",
        exitCode: exit.code,
        portReleased: true,
    ...
    throw new Error(
      `${error instanceof Error ? error.message : "MSAL cleanup probe failed."}\n${stdout}\n${stderr}`,
      { cause: error }
    );
  } finally {
