...
const renderManagementTools = (
  ...
  const lines = [
    ...
    "",
    `Status: Generated from the implemented ${catalog.managementMcp.toolCount}-tool management registry; F1 mock MCP is source-qualified locally; F2 mock-LLM definitions are management-only`,
  ...
  lines.push(
    ...
    `  provider data plane remains \`${catalog.future.mockLlmApis.providerDataPlane}\`,`,
    `  and deployed acceptance remains \`${catalog.future.mockLlmApis.deployedAcceptance}\`.`,
...
const renderLlmsIndex = (catalog: MockosManagementDocumentationCatalog): string =>
  [
    ...
    "- [Mock LLM guide](docs/mock-llm.md): configure source-implemented management",
    "  definitions and understand why the provider data plane remains unavailable.",
    ...
    "- F2 mock LLM provider data plane: unavailable; there is no callable OpenAI or",
    "  Anthropic route, model renderer, state/reset surface, paced stream, or deployment.",
