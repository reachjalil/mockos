...
const renderManagementTools = (
  ...
  const lines = [
    ...
    "",
    `Status: Generated from the implemented ${catalog.managementMcp.toolCount}-tool management registry; F1 mock MCP, bounded streaming OpenAI, and bounded non-streaming Anthropic data planes are source-qualified locally`,
...
const renderLlmsIndex = (catalog: MockosManagementDocumentationCatalog): string =>
  [
    ...
    "  its bounded streaming OpenAI or non-streaming Anthropic provider data plane.",
    ...
    "- F2 mock LLM provider data plane: bounded OpenAI Chat Completions including SSE,",
    "  plus non-streaming Anthropic Messages/model subsets, are source-qualified locally;",
    "  state/reset, observations, Anthropic streaming/betas, Cloud pinning, and deployment",
